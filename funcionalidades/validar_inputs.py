...
def id_correcto(valor: str) -> bool:
    return valor.isdigit() and int(valor) >= 1

def ingresar_texto(texto_mostrar:str) -> str:
    while True:
        try:
            texto = input(texto_mostrar).strip()
...
def ingresar_numero(texto_mostrar:str) -> str:
    while True:
        try:
            numero = input(texto_mostrar).strip()
...
def ingresar_id(texto_mostrar:str) -> str:
    while True:
        try:
            numero = input(texto_mostrar).strip()
            if id_correcto(numero):
                return numero
            else:
                raise ValueError
        except ValueError:
